...
for row in sheet.iter_rows(min_row=45, max_row=56, min_col=1, max_col=sheet.max_column):  
    ...
    folder_name = (str(row[2].value)+' '+str(date)+' '+str(row[4].value)+' '+str(row[6].value)+' '+str(row[5].value))  
    operator =  row[1].value
    child_dir = parent_dir / operator / "АВР_ДР" / "на оформлении" /  date_folder/ folder_name 
    child_dir.mkdir(parents=True, exist_ok=True) 
    child_photo = parent_dir / operator / "АВР_ДР" / "на оформлении" / date_folder / folder_name / "Фото"
    ...
    if (operator == 'Мегафон'):
        ...
        if sheet_name in wb.sheetnames:
            ...
            print(f"Ошибка: Лист '{sheet_name_akt}' не найден. Доступные листы: {wb.sheetnames}")
            print(f"Ошибка: Лист '{sheet_name_dr}' не найден. Доступные листы: {wb.sheetnames}")
            wb.close()
            exit()       
        # Вносим изменения 
